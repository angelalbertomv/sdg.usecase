...
try:
  ...
  file_format = (type = json);''')
    cs.execute('''merge into "SDG_DB"."SDG_USECASE"."sdl_sdgtestcase" des using (select PAYLOAD:currentPrice as price, BUSINESS_ID, load_date from "SDG_DB"."SDG_USECASE"."adl_sdgtestcase" t1 where load_date = (select max(load_date) as load_date from "SDG_DB"."SDG_USECASE"."adl_sdgtestcase" t2 where t2.business_id = t1.business_id )) ori
